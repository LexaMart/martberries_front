...
import { toggleProductToCart } from '../../../../store/reducers/StoreReducer/storeReducer';
...
export const StoreItem = ({ name, price, amount, id }: Product) => {
    ...
    const moveItemToCart = (): void => {
        if (orderNumber) {
            dispatch(toggleProductToCart({ name, price, orderNumber: orderNumber || 0, amount, id }))
    }
}

useEffect(() => {
    const userCartElement = userCart.find(el => el.id === id);
    if (!!userCartElement) {
        setInCart(true);
        setOrderNumber(userCartElement.orderNumber ? +userCartElement.orderNumber : 0);
    } else {
        setInCart(false);
    }
}, [id, userCart])
